...
// MARK: - 輔助工具與 GIF 視圖
func isMediaUrl(_ text: String) -> Bool {
    let clean = text.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    return clean.hasPrefix("http") && (clean.contains(".jpg") || clean.contains(".jpeg") || clean.contains(".png") || clean.contains(".gif") || clean.contains(".webp"))
}
...
struct GIFView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: WKWebView, context: Context) {
        let html = "<html><body style='margin:0;padding:0;background:transparent;display:flex;justify-content:center;align-items:center;'><img src='\(urlString)' style='width:100%;height:100%;object-fit:cover;'></body></html>"
...
struct ChatRoomView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let reply = viewModel.replyingToMessage {
                HStack {
                    Text("回覆：\(isMediaUrl(reply.text) ? "[圖片訊息]" : reply.text)").font(.caption).foregroundColor(.gray).lineLimit(1)
                    Spacer()
                    Button(action: { viewModel.replyingToMessage = nil }) { Image(systemName: "xmark.circle.fill").foregroundColor(.gray) }
                }.padding(.horizontal, 16).padding(.vertical, 8).background(Color(UIColor.systemBackground)); Divider()
            }
...
struct MessageBubbleView: View {
    ...
    private func BubbleContent() -> some View {
        ...
        VStack(alignment: message.isMine ? .trailing : .leading, spacing: 6) {
            ...
            if let replyText = message.replyToText, !replyText.isEmpty {
                Text("回覆：\(isMediaUrl(replyText) ? "[圖片訊息]" : replyText)")
                    .font(.caption2).foregroundColor(message.isMine ? .white.opacity(0.8) : .gray)
                    .lineLimit(1).truncationMode(.tail)
            ...
            Group {
                ...
                if isImageFile || isMediaUrl(cleanText) {
                    ...
                    if isGifUrl(targetImgUrl) {
                        ...
                        CachedImage(urlString: targetImgUrl, contentMode: .fill)
                            .frame(width: 220, height: 280)
                            .background(Color(UIColor.systemGray5))
                            .cornerRadius(12)
                            .clipped()
