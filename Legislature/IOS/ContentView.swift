        ...
        VStack(alignment: message.isMine ? .trailing : .leading, spacing: 6) {
            ...
            Group {
                if let imgUrl = message.fileUrl, message.fileType == "image" {
                    Button(action: { onPhotoTap(imgUrl) }) {
                        CachedImage(urlString: imgUrl, contentMode: .fill).frame(width: 220, height: 280).background(Color(UIColor.systemGray5)).cornerRadius(12).clipped()
                    }
                } else {
